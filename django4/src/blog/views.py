...
#검색
#특정 조건을 만족하는 POst객체를 리스트로 추출 후 화면에 출력
class PostSearch(ListView):
    template_name = 'blog/postsearch.html'
    model = Post
    context_object_name = 'objs'
    
    #사용자가 입력한 검색어를 포함한 Post객체가 추출되도록 get_queryset 함수를 오버라이드 
    #get_queryset 함수 : 특정 모델 클래스에서 객체를 꺼낼때 어떤 조건으로 꺼내야 하는지
    ...
    def get_queryset(self):
        ...
        #'search' key로 데이터를 추출하되, 해당key로 데이터가 존재하지않으면 빈 문자열을 반환하도록 설정  사전형 데이터에서 사용 할 수 있는 get 함수
        s=self.request.GET.get('search','')
        #추출한 데이터가 비 ㄴ문자열인 지 확인
        if s:
            #빈 문자열이 아닌경우 Post.objects.filter() 함수를 사용해서 검색어 맞는 객체들을 추출
            #filter(변수__조건 = 값) : 모델클래스 객체들이 특정 변수에 특정 조건을
            #만족하는지 확인해 만족하는 객체들을 리스트 형태로 반환하는 함수
            #조건에 들어가는 명령어는 django 라이브러리에서 지정함
            #icontains : 특정 변수에 우변에 값이 포함되 있으면 객체를 추출하도로 설정하는 조건
            #사용자가 입력한 키워드(s 변수) 를 제목에 포함하는Post 객체들을 추출
